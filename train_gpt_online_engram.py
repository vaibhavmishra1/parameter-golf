...
class Hyperparameters:
    ...
    # Validation-only mode loads an existing int8+zlib artifact and skips training.
    validate_only = bool(int(os.environ.get("VALIDATE_ONLY", os.environ.get("SKIP_TRAINING", "0"))))
    load_int8_path = os.environ.get("LOAD_INT8_PATH", "final_model.int8.ptz")

    # Causal online Engram evaluation. These knobs do not affect training.
    online_engram_enabled = bool(int(os.environ.get("ONLINE_ENGRAM_ENABLED", "1")))
    online_engram_orders = tuple(
        int(x) for x in os.environ.get("ONLINE_ENGRAM_ORDERS", "1,2,3").split(",") if x
    )
    ...
    online_engram_strict_batch_causal = bool(int(os.environ.get("ONLINE_ENGRAM_STRICT_BATCH_CAUSAL", "1")))
    online_engram_canonicalize = bool(int(os.environ.get("ONLINE_ENGRAM_CANONICALIZE", "1")))
...
def build_engram_canonical_lut(
    ...
    lut = np.empty((vocab_size,), dtype=np.uint32)
    for token_id in range(vocab_size):
        if token_id >= int(sp.vocab_size()):
            key = f"id:{token_id}"
        elif sp.is_control(token_id) or sp.is_unknown(token_id) or sp.is_unused(token_id) or sp.is_byte(token_id):
            key = f"id:{token_id}"
        else:
            piece = sp.id_to_piece(token_id)
            if piece.startswith("▁"):
                piece = piece[1:]
            piece = unicodedata.normalize("NFKC", piece).lower()
            key = f"text:{piece}" if piece else f"id:{token_id}"
        if key not in canonical_to_id:
            canonical_to_id[key] = len(canonical_to_id)
        lut[token_id] = canonical_to_id[key]
    return lut


def engram_previous_counts(values: np.ndarray) -> np.ndarray:
    if values.size == 0:
        return np.empty((0,), dtype=np.uint32)
    order = np.argsort(values, kind="stable")
    sorted_values = values[order]
    starts = np.empty_like(sorted_values, dtype=np.int64)
    group_starts = np.concatenate(
        (
            np.array([0], dtype=np.int64),
            np.nonzero(sorted_values[1:] != sorted_values[:-1])[0].astype(np.int64) + 1,
        )
    )
    group_lengths = np.diff(np.concatenate((group_starts, np.array([values.size], dtype=np.int64))))
    starts[:] = np.repeat(group_starts, group_lengths)
    sorted_counts = (np.arange(values.size, dtype=np.int64) - starts).astype(np.uint32, copy=False)
    out = np.empty((values.size,), dtype=np.uint32)
    out[order] = sorted_counts
    return out


def engram_hash_keys(
    ...
    seed = (1469598103934665603 ^ (0x9E3779B97F4A7C15 * (order + 17 * head))) & ((1 << 64) - 1)
    h = np.full(
        (num_targets,),
        np.uint64(seed),
        dtype=np.uint64,
    ...
    for offset in range(order):
        src_start = target_start - order + offset
        vals = np.empty((num_targets,), dtype=np.uint64)
        missing = min(max(-src_start, 0), num_targets)
        if missing:
            vals[:missing] = np.uint64(sentinel + offset)
        if missing < num_targets:
            src = canonical_tokens[src_start + missing : src_start + num_targets]
            vals[missing:] = src.astype(np.uint64, copy=False)
        vals = vals + np.uint64(0x517CC1B727220A95 + 0x9E3779B1 * (offset + 1))
        h ^= vals
        h *= prime
    h ^= h >> np.uint64(33)
    h *= np.uint64(0xFF51AFD7ED558CCD)
    h ^= h >> np.uint64(33)
    return (h % np.uint64(buckets)).astype(np.int64, copy=False)


def eval_val_online_engram(
    args: Hyperparameters,
    model: "GPT",
    rank: int,
    device: torch.device,
    ...
    if rank != 0:
        return None
    if args.online_engram_alpha < 0.0 or args.online_engram_alpha > 1.0:
        raise ValueError("ONLINE_ENGRAM_ALPHA must be in [0, 1]")
    if args.online_engram_heads <= 0 or args.online_engram_buckets <= 0:
        raise ValueError("ONLINE_ENGRAM_HEADS and ONLINE_ENGRAM_BUCKETS must be positive")
    if args.online_engram_min_count < 1:
    ...
    if any(order <= 0 for order in orders):
        raise ValueError(f"ONLINE_ENGRAM_ORDERS must be positive integers, got {orders}")
    ...
    if table_bytes > args.online_engram_max_table_bytes:
        raise ValueError(
            "Online Engram count tables would use "
            f"{table_bytes:,} bytes; raise ONLINE_ENGRAM_MAX_TABLE_BYTES or reduce "
            "ONLINE_ENGRAM_BUCKETS/HEADS/ORDERS/VOCAB_SIZE."
        )
    counts = np.zeros(
        (num_tables, args.online_engram_buckets, vocab_size),
        dtype=np.uint32,
    )
    ...
    base_bytes = base_bytes_lut.detach().cpu().numpy().astype(np.int64, copy=False)
    has_leading_space = has_leading_space_lut.detach().cpu().numpy().astype(np.bool_, copy=False)
    is_boundary = is_boundary_token_lut.detach().cpu().numpy().astype(np.bool_, copy=False)

    total_targets = ((val_tokens.numel() - 1) // args.train_seq_len) * args.train_seq_len
    if args.online_engram_max_tokens > 0:
        total_targets = min(total_targets, args.online_engram_max_tokens)
        total_targets = (total_targets // args.train_seq_len) * args.train_seq_len
    if total_targets <= 0:
        raise ValueError("ONLINE_ENGRAM_MAX_TOKENS leaves no complete validation sequence")
    ...
    table_specs = [(order, head) for order in orders for head in range(args.online_engram_heads)]

    was_training = model.training
    model.eval()
    with torch.inference_mode():
        for seq_start in range(0, total_seqs, chunk_seqs):
            ...
            targets_t = y.reshape(-1)
            log_probs = F.log_softmax(logits.reshape(-1, vocab_size).float(), dim=-1)
            model_nll_t = -log_probs.gather(1, targets_t[:, None]).squeeze(1)
            model_prob = torch.exp(-model_nll_t).detach().cpu().numpy().astype(np.float64, copy=False)
            normal_nll_sum += float(model_nll_t.double().sum().item())

            targets = targets_t.detach().cpu().numpy().astype(np.int64, copy=False)
            n = int(targets.size)
            target_start = raw_start + 1
            prev_ids = val_np[target_start - 1 : target_start + n - 1]
            token_bytes = base_bytes[targets] + (has_leading_space[targets] & ~is_boundary[prev_ids])
            byte_count += float(token_bytes.sum())
            ...
            for order, head in table_specs:
                keys = engram_hash_keys(
                    ...
                    order,
                    head,
                    args.online_engram_buckets,
                    sentinel + max_order * head,
                )
                ...
                if args.online_engram_strict_batch_causal:
                    ...
                    total_before += engram_previous_counts(keys)

                valid = total_before >= args.online_engram_min_count
                if valid.any():
...
class GPT(nn.Module):
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        return logits.reshape(*input_ids.shape, -1)
...
def main() -> None:
    ...
    engram_canonical_lut = build_engram_canonical_lut(sp, args.vocab_size, args.online_engram_canonicalize)
    log0(f"val_bpb:enabled tokenizer_kind=sentencepiece tokenizer_path={args.tokenizer_path}")
    log0(f"validate_only:{args.validate_only} load_int8_path:{args.load_int8_path}")
    log0(
        "online_engram:"
    ...
    if args.validate_only:
        load_path = Path(args.load_int8_path)
        if not load_path.exists():
            raise FileNotFoundError(f"VALIDATE_ONLY requested but LOAD_INT8_PATH does not exist: {load_path}")
        with open(load_path, "rb") as f:
            quant_blob_disk = f.read()
        quant_state = torch.load(io.BytesIO(zlib.decompress(quant_blob_disk)), map_location="cpu")
        base_model.load_state_dict(dequantize_state_dict_int8(quant_state), strict=True)
        log0(f"validate_only:loaded_int8 path={load_path} bytes:{load_path.stat().st_size}")
        log0(f"model_params:{n_params}")

        torch.cuda.synchronize()
        t_eval = time.perf_counter()
        normal_val_loss, normal_val_bpb = eval_val(
            args,
            model,
            rank,
            world_size,
        ...
        torch.cuda.synchronize()
        log0(
            f"validate_only_normal val_loss:{normal_val_loss:.4f} val_bpb:{normal_val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_eval):.0f}ms"
        )
        log0(f"validate_only_normal_exact val_loss:{normal_val_loss:.8f} val_bpb:{normal_val_bpb:.8f}")

        if args.online_engram_enabled:
            torch.cuda.synchronize()
            t_engram = time.perf_counter()
            online_result = eval_val_online_engram(
                args,
                base_model,
                rank,
                device,
