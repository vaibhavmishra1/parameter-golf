"""
In-Place TTT variant of train_gpt.py.

Architecture, hyperparameters, training loop, quantization: ALL IDENTICAL to baseline.
...
Apple-to-apple comparison:
  Both val_bpb (standard) and ttt_val_bpb (TTT-enhanced) are reported at final eval.
  Training is identical — only evaluation changes.
"""
...
class Hyperparameters:
    ...
    # -------------------------------------------------------
    # IN-PLACE TTT HYPERPARAMETERS (new — training unchanged)
    # -------------------------------------------------------
    # ttt_enabled:    Run TTT eval in addition to standard eval at final checkpoint.
    # ttt_lr:         Step size for W_down fast-weight update (η in the paper).
    # ttt_chunk_size: Tokens per TTT chunk. Larger = more context per update but less adaptation.
    #                 Paper uses 2K-4K (LaCT). For seq_len=1024, 64-256 is a sensible range.
    # ttt_also_mid:   Also run TTT eval at intermediate val checkpoints (slower but informative).
    ttt_enabled    = bool(int(os.environ.get("TTT_ENABLED", "1")))
    ttt_lr         = float(os.environ.get("TTT_LR", "0.01"))
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", "64"))
    ttt_also_mid   = bool(int(os.environ.get("TTT_ALSO_MID", "0")))
...
# -------------------------------------------------------
# STANDARD EVAL (identical to baseline — for comparison)
# -------------------------------------------------------
...
def eval_val_with_ttt(
    args: Hyperparameters,
    base_model: nn.Module,          # Raw GPT (not DDP/compiled) — we update its weights
    rank: int,
    world_size: int,
    device: torch.device,
    grad_accum_steps: int,
    ...
    if local_batch_tokens < args.train_seq_len:
        raise ValueError(
            "VAL_BATCH_SIZE must provide at least one sequence per rank for TTT eval"
        )
    local_batch_seqs = local_batch_tokens // args.train_seq_len
    total_seqs = (val_tokens.numel() - 1) // args.train_seq_len
    seq_start = (total_seqs * rank) // world_size
    ...
    for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
        batch_seq_end = min(batch_seq_start + local_batch_seqs, seq_end)
        n_seqs = batch_seq_end - batch_seq_start
...
# -----------------------------
# POST-TRAINING QUANTIZATION (identical to baseline)
# -----------------------------
...
# -----------------------------
# TRANSFORMER MODULES (identical to baseline)
# -----------------------------
...
class MLP(nn.Module):
    # relu^2 MLP — identical to baseline.
    # The TTT hook captures `inp[0]` to `self.proj` in eval_val_with_ttt,
    # which equals relu(fc(x))^2.  No code change needed here.
...
# -----------------------------
# TRAINING (identical to baseline, TTT eval added at final checkpoint)
# -----------------------------
...
def main() -> None:
    ...
    log0(f"ttt_enabled:{args.ttt_enabled} ttt_lr:{args.ttt_lr} ttt_chunk_size:{args.ttt_chunk_size}")
    ...
    while True:
        ...
        if should_validate:
            ...
            # Standard eval (identical to baseline)
            ...
            # TTT eval — run at intermediate checkpoints only if ttt_also_mid=True,
            # always run at the final step (last_step)
            run_ttt = args.ttt_enabled and (last_step or args.ttt_also_mid)
            if run_ttt:
                torch.cuda.synchronize()
                ...
                bpb_delta = ttt_val_bpb - val_bpb
                log0(
                    f"step:{step}/{args.iterations} ttt_val_loss:{ttt_val_loss:.4f} "
                    f"ttt_val_bpb:{ttt_val_bpb:.4f} bpb_delta:{bpb_delta:+.4f} "
                    f"ttt_eval_ms:{ttt_eval_ms:.0f}ms "
                    f"[ttt_lr={args.ttt_lr} chunk={args.ttt_chunk_size}]"
                )
    ...
    # -----------------------------
    # SERIALIZATION + ROUNDTRIP (identical to baseline)
    # -----------------------------
    ...
    # -- TTT roundtrip eval (applied AFTER quantization, on the quantized weights) --
    if args.ttt_enabled:
        torch.cuda.synchronize()
        t_ttt_final = time.perf_counter()
        ttt_q_val_loss, ttt_q_val_bpb = eval_val_with_ttt(
            args, base_model, rank, world_size, device, grad_accum_steps,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            ttt_lr=args.ttt_lr,
            ttt_chunk_size=args.ttt_chunk_size,
        ...
        torch.cuda.synchronize()
        ttt_final_ms = 1000.0 * (time.perf_counter() - t_ttt_final)
        bpb_gain = q_val_bpb - ttt_q_val_bpb  # positive = TTT helped
        log0(
            f"final_int8_zlib_ttt val_loss:{ttt_q_val_loss:.4f} val_bpb:{ttt_q_val_bpb:.4f} "
            f"bpb_gain_vs_standard:{bpb_gain:+.4f} "
            f"eval_time:{ttt_final_ms:.0f}ms "
            f"[ttt_lr={args.ttt_lr} chunk={args.ttt_chunk_size}]"
        )
        log0(f"final_int8_zlib_ttt_exact val_loss:{ttt_q_val_loss:.8f} val_bpb:{ttt_q_val_bpb:.8f}")
