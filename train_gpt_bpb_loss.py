...
def sum_target_utf8_bytes(
    ...
    """UTF-8 byte count for target tokens (same rule as eval_val). Scalar float64 on x's device."""
...
def main() -> None:
    ...
    log0(
        ...
        "logged train_nats_per_byte ≈ val_bpb * ln(2); val_loss still mean token CE"
    )
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            b_total = torch.zeros((), device=device, dtype=torch.float64)
            for _ in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                batches.append((x, y))
                b_total += sum_target_utf8_bytes(
                    x, y, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut
                )
            inv_b = (1.0 / b_total.clamp(min=1.0)).to(dtype=torch.float32)
    ...
    while True:
        ...
        for _ in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            batches.append((x, y))
            b_total += sum_target_utf8_bytes(x, y, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
        inv_b = (1.0 / b_total.clamp(min=1.0)).to(dtype=torch.float32)
        ...
        if should_log_train:
            log0(
                f"step:{step}/{args.iterations} train_nats_per_byte:{train_loss.item():.4f} "
