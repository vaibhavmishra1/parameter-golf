...
class Hyperparameters:
    ...
    # Eval-only, non-learned online Engram. Training and serialization remain
    # identical to train_gpt.py; this only changes the extra diagnostic eval.
    engram_eval = bool(int(os.environ.get("ENGRAM_EVAL", "1")))
    engram_order = int(os.environ.get("ENGRAM_ORDER", 4))
...
class OnlineEngramLite:
    ...
    def observe_prefix(self, token: int) -> None:
        self.packed_key = ((self.packed_key << self.token_bits) | int(token)) & self.key_mask
        self.hist_len = min(self.hist_len + 1, self.order)

    def bucket(self) -> int:
        x = (self.packed_key ^ (self.hist_len * 0x9E3779B97F4A7C15)) & ((1 << 64) - 1)
        x ^= x >> 30
        x = (x * 0xBF58476D1CE4E5B9) & ((1 << 64) - 1)
        x ^= x >> 27
        x = (x * 0x94D049BB133111EB) & ((1 << 64) - 1)
        x ^= x >> 31
        return int(x & self.bucket_mask)

    def score_then_update(self, target: int, neural_prob: float) -> float:
        target = int(target)
        b = self.bucket()
        key_total = int(self.total[b])
        pair_count = int(self.next_counts[b, target])
        if self.unigram_total > 0:
            unigram_prob = (float(self.unigram[target]) + 1.0 / self.vocab_size) / (self.unigram_total + 1.0)
        else:
        ...
        self.observe_prefix(target)
        return max(prob, 1e-45)


def _power_of_two_at_most(n: int) -> int:
...
def eval_val_online_engram(
    ...
    max_buckets = max(args.engram_max_table_bytes // max(vocab_size * np.dtype(np.uint32).itemsize, 1), 1)
    buckets = _power_of_two_at_most(min(args.engram_buckets, max_buckets))
    engram = OnlineEngramLite(
        vocab_size=vocab_size,
        order=args.engram_order,
        buckets=buckets,
        alpha=args.engram_alpha,
    ...
    total_pairs = val_tokens.numel() - 1
    if args.engram_eval_tokens > 0:
        total_pairs = min(total_pairs, args.engram_eval_tokens)
    total_pairs = (total_pairs // args.train_seq_len) * args.train_seq_len
    if total_pairs <= 0:
        raise ValueError("ENGRAM_EVAL_TOKENS leaves no full validation sequences to score")

    base_bytes_np = base_bytes_lut.detach().cpu().numpy()
    has_space_np = has_leading_space_lut.detach().cpu().numpy()
    boundary_np = is_boundary_token_lut.detach().cpu().numpy()
    model.eval()
    engram.observe_prefix(int(val_tokens[0].item()))

    nn_loss_sum = 0.0
    engram_loss_sum = 0.0
    byte_count = 0.0
    token_count = 0
    num_seqs = total_pairs // args.train_seq_len
    with torch.inference_mode():
        for batch_seq_start in range(0, num_seqs, local_batch_seqs):
            batch_seq_end = min(batch_seq_start + local_batch_seqs, num_seqs)
            raw_start = batch_seq_start * args.train_seq_len
            raw_end = batch_seq_end * args.train_seq_len + 1
            local = val_tokens[raw_start:raw_end].to(device=device, dtype=torch.int64, non_blocking=True)
            x = local[:-1].reshape(-1, args.train_seq_len)
            ...
            p_nn = logp.exp().detach().cpu().reshape(-1).numpy()
            y_np = y.detach().cpu().reshape(-1).numpy()
            x_np = x.detach().cpu().reshape(-1).numpy()
            token_bytes = base_bytes_np[y_np].astype(np.int64)
            token_bytes += np.logical_and(has_space_np[y_np], np.logical_not(boundary_np[x_np])).astype(np.int64)
            byte_count += float(token_bytes.sum())
            nn_loss_sum += float((-logp).sum().item())
            for target, prob in zip(y_np, p_nn):
                engram_loss_sum -= math.log(engram.score_then_update(int(target), float(prob)))
            token_count += int(y_np.size)

    nn_loss = nn_loss_sum / token_count
    engram_loss = engram_loss_sum / token_count
    tokens_per_byte = token_count / byte_count
    stats = {
        "tokens": float(token_count),
        "bytes": float(byte_count),
        "buckets": float(buckets),
...
def load_int8_checkpoint(path: str | Path) -> dict[str, Tensor]:
...
def main() -> None:
    ...
    def run_engram_eval(prefix: str) -> None:
        if not args.engram_eval:
            log0(f"{prefix}:disabled")
            return
        torch.cuda.synchronize()
        log0(
            f"{prefix}_start order:{args.engram_order} requested_buckets:{args.engram_buckets} "
            f"max_table_bytes:{args.engram_max_table_bytes} eval_tokens:{args.engram_eval_tokens}"
        )
        t_engram = time.perf_counter()
        nn_loss, nn_bpb, engram_loss, engram_bpb, engram_stats = eval_val_online_engram(
            args,
            base_model,
            rank,
            world_size,
    ...
    if args.eval_only:
        ...
        if not eval_model_path.is_file():
            raise FileNotFoundError(f"EVAL_MODEL_PATH does not exist: {eval_model_path}")
        log0(f"eval_only:enabled model_path:{eval_model_path}")
        base_model.load_state_dict(load_int8_checkpoint(eval_model_path), strict=True)
        if distributed:
            dist.barrier()
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(
            f"eval_only_normal val_loss:{val_loss:.4f} val_bpb:{val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_eval):.0f}ms"
        )
        log0(f"eval_only_normal_exact val_loss:{val_loss:.8f} val_bpb:{val_bpb:.8f}")
        run_engram_eval("eval_only_engram_lite")
        if distributed:
    ...
    if distributed:
        dist.barrier()
    base_model.load_state_dict(load_int8_checkpoint("final_model.int8.ptz"), strict=True)
    torch.cuda.synchronize()
