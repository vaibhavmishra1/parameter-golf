#!/usr/bin/env python3
"""
train_gpt_mlx_ngpt.py — nGPT (Normalized Transformer on the Hypersphere) experiment.

Paper: "nGPT: Normalized Transformer on the Hypersphere" (arXiv:2410.01131).
Key claim: 4–20× fewer training steps to reach the same loss as a standard Transformer,
because all vectors evolve on the unit hypersphere with controlled step sizes.

Changes vs. baseline:
  1. unit_norm() replaces rms_norm() on the residual stream:
       each token vector is L2-normalized to ||x||=1 after every residual update.
  2. Weight rows of Q/K/V/proj/fc/proj are L2-normalized in the forward pass.
       (NormalizedCastedLinear instead of CastedLinear for all matrix weights.)
  3. attn_scale / mlp_scale (step-size "alphas") are initialized small (NGPT_ALPHA_INIT=0.05)
       instead of 1.0, so early updates are tiny perturbations on the sphere.
  4. resid_mix is removed; nGPT doesn't mix with a distant anchor x0.
       (x0 is still passed for API compatibility but ignored in the block forward.)
  5. Initial embedding is unit-normalized instead of rms-normalized.

No extra parameters beyond small alpha scalars. This is a pure training-dynamics change.

Run smoke test:
  RUN_ID=ngpt_smoke ITERATIONS=200 TRAIN_BATCH_TOKENS=8192 \
  VAL_LOSS_EVERY=0 VAL_BATCH_SIZE=8192 python3 train_gpt_mlx_ngpt.py
"""
...
class Hyperparameters:
    ...
    # nGPT: per-dim step-size alpha for attn and mlp residual updates.
    # Paper initializes to ~1/num_layers. Larger → faster change per step but less stable early.
    ngpt_alpha_init: float = float(os.environ.get("NGPT_ALPHA_INIT", "0.0"))  # 0 = auto = 1/num_layers
    # Logit temperature: hidden states are unit-norm so raw logits are cosine similarities in [-1,1].
    # This learnable scalar scales them to a useful CE range. Paper uses s_z initialized ~1/sqrt(d).
    ngpt_logit_temp_init: float = float(os.environ.get("NGPT_LOGIT_TEMP_INIT", "0.0"))  # 0 = auto = sqrt(dim)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,q_gain,skip_weight,skip_weights,logit_temp,resid_mix",
    ).split(",")
    if pattern
)
...
def unit_norm(x: mx.array, eps: float = 1e-8) -> mx.array:
    """Normalize to unit L2 norm along the last axis (place x on the unit hypersphere)."""
    return (x * mx.rsqrt(mx.sum(x * x, axis=-1, keepdims=True) + eps)).astype(x.dtype)
...
def _row_normalize(w: mx.array) -> mx.array:
    """Normalize each row of a 2D weight matrix to unit L2 norm."""
    return w * mx.rsqrt(mx.sum(w * w, axis=-1, keepdims=True) + 1e-8)


def _tangent_project(h: mx.array, z: mx.array) -> mx.array:
    """Project z onto the tangent plane of the unit sphere at h.

    This is the key nGPT operation: strip the component of z that points
    along h so the update moves ALONG the sphere surface rather than
    radially away from it.  result = z - h * <h, z>
    """
    dot = mx.sum(h * z, axis=-1, keepdims=True)
    return z - h * dot


class CastedLinear(nn.Module):
    """Baseline linear (no weight normalization). Used for V and proj where
    the paper does NOT require row-norm, or when weight-norm hurts init."""
...
class NormalizedCastedLinear(nn.Module):
    """Linear layer with weight-row normalization in the forward pass (nGPT).

    Weight rows are L2-normalized every forward call. Gradients update the raw
    weights; normalization is re-applied on the next forward. This keeps the
    linear map operating between unit-norm spaces.
    """
    def __init__(self, in_dim: int, out_dim: int):
        super().__init__()
        self.weight = nn.Linear(in_dim, out_dim, bias=False).weight.astype(mx.float32)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        return x @ _row_normalize(self.weight).astype(x.dtype).T
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float):
        ...
        self.c_q = NormalizedCastedLinear(dim, dim)
        self.c_k = NormalizedCastedLinear(dim, kv_dim)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, leaky_slope: float = 0.5):
        ...
        self.fc = NormalizedCastedLinear(dim, hidden)
        self.proj = CastedLinear(hidden, dim)

    def __call__(self, x: mx.array) -> mx.array:
        h = nn.leaky_relu(self.fc(x), negative_slope=self.leaky_slope)
        return self.proj(h * h)


class Block(nn.Module):
    """nGPT block: tangent-plane residual updates on the unit hypersphere.

    From the paper (Algorithm 1):
      z_attn = Attn(RMSNorm(h))
      h = Normalize(h + s_attn * (z_attn - h * <h, z_attn>))   # tangent projection
      z_mlp  = MLP(RMSNorm(h))
      h = Normalize(h + s_mlp  * (z_mlp  - h * <h, z_mlp>))    # tangent projection

    The tangent projection (z - h*dot(h,z)) removes the radial component of the
    sublayer output so the update moves along the sphere surface. Without this,
    naive add-and-normalize fights the normalization instead of doing geodesic steps.
    """
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 rope_base: float, qk_gain_init: float, alpha_init: float):
        super().__init__()
        self.attn_norm = RMSNormNoWeight()
        self.mlp_norm = RMSNormNoWeight()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = MLP(dim, mlp_mult)
        self.attn_scale = mx.full((dim,), alpha_init, dtype=mx.float32)
        self.mlp_scale = mx.full((dim,), alpha_init, dtype=mx.float32)
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        x = unit_norm(x + self.attn_scale.astype(x.dtype)[None, None, :] * _tangent_project(x, z_attn))

        z_mlp = self.mlp(self.mlp_norm(x))
        x = unit_norm(x + self.mlp_scale.astype(x.dtype)[None, None, :] * _tangent_project(x, z_mlp))
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int,
                 ...
                 tied_embed_init_std: float, qk_gain_init: float, ngpt_alpha_init: float,
                 ngpt_logit_temp_init: float):
        super().__init__()
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, ngpt_alpha_init)
            for _ in range(num_layers)
        ]
        # No final rms_norm: hidden states are already unit-norm from the blocks.
        # rms_norm on unit-norm vectors multiplies by sqrt(dim) which would blow up
        # logits through softcap and saturate everything.
        # Learnable logit temperature: hidden states are unit-norm, so raw logits
        # are cosine similarities in [-1, 1]. We scale them up by this temperature
        # to produce a useful cross-entropy loss landscape.
        self.logit_temp = mx.array(ngpt_logit_temp_init, dtype=mx.float32)

        # Zero-initialize output projections (attn.proj, mlp.proj) — same as baseline.
        # These are plain CastedLinear (NOT NormalizedCastedLinear) so zero-init is safe.
        # This makes the model start as identity: each block's sublayer output is zero,
        # so unit_norm(h + alpha * tangent_project(h, 0)) = unit_norm(h) = h.
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        x = unit_norm(self.tok_emb(input_ids).astype(COMPUTE_DTYPE))
        x0 = x
        skips: list[mx.array] = []
        ...
        for i in range(self.num_decoder_layers):
            if skips:
                x = unit_norm(x + self.skip_weights[i].astype(x.dtype)[None, None, :] * skips.pop())
            x = self.blocks[self.num_encoder_layers + i](x, x0)
        # x is already unit-norm from the last block. Do NOT rms_norm here:
        # rms_norm would multiply by sqrt(dim) ≈ 22.6, making logits saturate softcap.
        return x

    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)
        # Row-normalize embedding weights for cosine-similarity logits, then scale
        w_norm = _row_normalize(self.tok_emb.weight.astype(x.dtype))
        temp = self.logit_temp.astype(x.dtype)
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits = self.softcap(temp * (x @ w_norm.T))
            return nn.losses.cross_entropy(logits.astype(mx.float32), y, reduction="mean")
        loss_sum = mx.array(0.0, dtype=mx.float32)
        n = int(x.shape[0])
        for s in range(0, n, self.logit_chunk_tokens):
            e = min(s + self.logit_chunk_tokens, n)
            logits = self.softcap(temp * (x[s:e] @ w_norm.T))
            loss_sum = loss_sum + nn.losses.cross_entropy(logits.astype(mx.float32), y[s:e], reduction="sum")
        return loss_sum / float(n)


# ==============================================================================
# OPTIMIZERS (MUON + ADAM) — unchanged from baseline
# ==============================================================================
class Muon:
...
class SplitOptimizers:
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.scalar_keys = [
            k for k, p in params.items()
            if k == "skip_weights" or k == "logit_temp" or (k.startswith("blocks.") and (p.ndim < 2 or any(pat in k for pat in CONTROL_TENSOR_NAME_PATTERNS)))
        ]
...
# ==============================================================================
# QUANTIZATION (INT8 + ZLIB) — unchanged from baseline
# ==============================================================================
...
INT8_KEEP_FLOAT_FP32_NAME_PATTERNS: tuple[str, ...] = ()
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    return {"quantized": quantized, "scales": scales, "dtypes": dtypes, "passthrough": passthrough, "passthrough_orig_dtypes": passthrough_orig_dtypes, "qmeta": qmeta}, stats
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q_np in quant_obj["quantized"].items():
...
def validate_dataset_tokenizer_pair(data_path: str, tokenizer_path: str) -> tuple[str, int, int | None]:
    ...
    if expected_train_files is not None:
        ...
        if actual_train_files > expected_train_files:
            raise ValueError(f"too many train shards found {actual_train_files}, manifest says {expected_train_files}")
...
def eval_val(args: Hyperparameters, compiled_loss, val_tokens: np.ndarray,
    ...
    for batch_idx, batch_seq_start in enumerate(range(0, total_seqs, val_batch_seqs), start=1):
        ...
        if log_fn is not None and total_batches > 1 and (batch_idx == 1 or batch_idx == total_batches or batch_idx % 2500 == 0):
...
def main() -> None:
    ...
    model = GPT(
        ...
        qk_gain_init=args.qk_gain_init, ngpt_alpha_init=args.ngpt_alpha_init,
        ngpt_logit_temp_init=args.ngpt_logit_temp_init,
    )
    ...
    log(f"model_params:{n_params} vocab_size:{args.vocab_size} layers:{args.num_layers} dim:{args.model_dim} heads:{args.num_heads} kv_heads:{args.num_kv_heads} seq_len:{args.train_seq_len}")
    log(f"iterations:{args.iterations} train_batch_tokens:{args.train_batch_tokens} grad_accum_steps:{args.grad_accum_steps} microbatch_tokens:{args.microbatch_tokens} val_batch_size:{args.val_batch_size} warmup_steps:{args.warmup_steps} max_wallclock_seconds:{args.max_wallclock_seconds:.3f}")
    alpha_actual = args.ngpt_alpha_init if args.ngpt_alpha_init > 0 else 1.0 / max(args.num_layers, 1)
    temp_actual = args.ngpt_logit_temp_init if args.ngpt_logit_temp_init > 0 else math.sqrt(args.model_dim)
    log(f"arch:ngpt alpha_init:{alpha_actual:.4f} logit_temp_init:{temp_actual:.2f} tangent_project:True weight_norm:qk_fc_rows residual:unit_sphere")
    ...
    if args.warmup_steps > 0:
        ...
        mx.eval(compiled_loss(mx.array(warm_chunk[:-1].reshape(-1, args.train_seq_len), dtype=mx.int32), mx.array(warm_chunk[1:].reshape(-1, args.train_seq_len), dtype=mx.int32)))
        mx.synchronize()
        train_loader = TokenLoader(args.train_files, log_fn=log, dataset_name=dataset_name)
    ...
    log(f"serialized_model_int8_zlib:{quant_path.stat().st_size} bytes (payload:{quant_stats['int8_payload_bytes']} raw_pickle:{len(quant_raw)} payload_ratio:{ratio:.2f}x)")
