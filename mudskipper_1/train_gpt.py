...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if weights is None:
            # Validation and unweighted training go through the unchanged path.
            return F.cross_entropy(logits.float(), targets, reduction="mean")
        # Mudskipper v1: weights are pre-normalized to mean ≈ 1.0 by the scorer,
        # so this stays gradient-magnitude-comparable to the unweighted version.
        per_token_loss = F.cross_entropy(logits.float(), targets, reduction="none")
        w = weights.reshape(-1).to(per_token_loss.dtype)
...
def main() -> None:
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                # Mirror the weighted forward path during warmup so the compile cache
                # matches what the real training loop will use.
                warmup_weights = scorer.weights(x, y) if scorer is not None else None
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, weights=warmup_weights)
    ...
    while True:
        ...
        last_weight_diag: dict[str, float] | None = None
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            if scorer is not None:
                weights = scorer.weights(x, y)
            else:
                weights = None
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, weights=weights)
