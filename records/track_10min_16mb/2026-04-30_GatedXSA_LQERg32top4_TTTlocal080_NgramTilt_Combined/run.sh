#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Combined Best-of-Best run script
#
...
TTT_ENABLED=1
TTT_EVAL_ONLY="${TTT_EVAL_ONLY:-1}" # set 1 to reuse final_model.int6.ptz and test eval-side changes only
PHASED_TTT_NUM_PHASES=1          # phased TTT is auto-on whenever NUM_PHASES > 0
PHASED_TTT_PREFIX_DOCS=1000
TTT_LORA_RANK=80
EVAL_SEQ_LEN=2560
TTT_EVAL_SEQ_LEN=2560
TTT_SHORT_SCORE_FIRST_ENABLED="${TTT_SHORT_SCORE_FIRST_ENABLED:-1}"
TTT_SHORT_SCORE_FIRST_STEPS="${TTT_SHORT_SCORE_FIRST_STEPS:-256:8,2000:24}"
TTT_SHORT_EVAL_BUDGET_SECONDS="${TTT_SHORT_EVAL_BUDGET_SECONDS:-565}"
...
export SEED RUN_ID DATA_PATH TOKENIZER_PATH \
       ...
       TTT_ENABLED TTT_EVAL_ONLY PHASED_TTT_NUM_PHASES PHASED_TTT_PREFIX_DOCS \
       TTT_LORA_RANK EVAL_SEQ_LEN TTT_EVAL_SEQ_LEN \
       TTT_SHORT_SCORE_FIRST_ENABLED TTT_SHORT_SCORE_FIRST_STEPS \
       TTT_SHORT_EVAL_BUDGET_SECONDS \
