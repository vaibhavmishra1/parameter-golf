"""Differential Attention variant of the Megakernel submission.
Builds on the TripleLoop+ParallelResiduals SOTA baseline.

Key idea (from "Differential Transformer", Ye et al., 2024 — Microsoft Research):
  Instead of one softmax attention map, compute TWO:
    A1 = softmax(Q1·K1ᵀ / √d),  A2 = softmax(Q2·K2ᵀ / √d)
    output = (A1 - λ·A2) @ V
  The subtraction cancels common "attention noise" (uniform background attention),
  forcing the model to focus sharper on truly relevant tokens.

Architecture changes vs. standard transformer:
  - sub_head_dim = head_dim // 2  (each Q/K is split into two halves)
  - c_q: dim → 2*num_heads*sub_head_dim  (same total params — two Q halves per head)
  - c_k: dim → 2*num_kv_heads*sub_head_dim (same total params — two K halves per head)
  - c_v: dim → num_kv_heads*sub_head_dim   (HALF the V params — reduced head_dim)
  - proj: num_heads*sub_head_dim → dim     (HALF the proj params — half input width)
  - λ_coef: [num_heads] learnable per-head scalars (sigmoid-gated)

Parameter budget analysis per attention block (model_dim=512, num_heads=8, num_kv_heads=4):
  Standard:   Q(262K) + K(131K) + V(131K) + proj(262K) = 786K params
  Diff-Attn:  Q(262K) + K(131K) + V(65.5K) + proj(131K) + λ(8) ≈ 590K params
  SAVINGS ≈ 196K params per block × 11 blocks ≈ 2.16M params freed.

This budget can be reinvested in a wider model (e.g., model_dim=576 instead of 512)
or more layers. The sharper attention should also improve convergence speed per step.

Changes vs. base:
  1. Hyperparameters: diff_attn (enable flag)
  2. New DiffCausalSelfAttention class replacing CausalSelfAttention
  3. Block: uses DiffCausalSelfAttention when diff_attn=True
  4. GPT.__init__: updated RoPE initialization for sub_head_dim
"""
import collections
import copy
import glob
...
class Hyperparameters():
    ...
    diff_attn = bool(int(os.environ.get('DIFF_ATTN', '1')))  # enable differential attention
...
class DiffCausalSelfAttention(nn.Module):
    ...
    Q and K are each split into two halves (Q1/Q2, K1/K2) of sub_head_dim = head_dim//2.
    V also uses sub_head_dim, halving V and proj params while keeping Q/K params the same.
    Two separate flash-attention calls produce y1 and y2; the output is (y1 - λ·y2).
    A per-head learnable λ (sigmoid-gated) controls the subtraction strength.
    """
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
                 rope_base: float, qk_gain_init: float, train_seq_len: int,
                 layer_idx: int = 0):
        ...
        self.num_heads = num_heads
        self.num_kv_heads = num_kv_heads
        self.head_dim = dim // num_heads          # full head_dim (kept for compatibility)
        self.sub_head_dim = self.head_dim // 2    # each Q1/Q2/K1/K2/V slice dimension
        if self.head_dim % 2 != 0:
            raise ValueError("head_dim must be even for DiffAttn sub-split")

        # c_q outputs two Q halves stacked: shape [B,T, num_heads, 2*sub_head_dim]
        # same total params as standard c_q (2*sub_head_dim == head_dim)
        self.c_q = CastedLinear(dim, num_heads * 2 * self.sub_head_dim, bias=False)
        # c_k outputs two K halves stacked: same params as standard c_k
        kv_sub_dim = num_kv_heads * self.sub_head_dim
        self.c_k = CastedLinear(dim, num_kv_heads * 2 * self.sub_head_dim, bias=False)
        # c_v uses sub_head_dim → half the V params
        self.c_v = CastedLinear(dim, kv_sub_dim, bias=False)
        # proj input is num_heads*sub_head_dim → half the proj params
        self.proj = CastedLinear(num_heads * self.sub_head_dim, dim, bias=False)
        self.proj._zero_init = True

        # Per-head gain (applied to both Q1 and Q2)
        self.q_gain = nn.Parameter(torch.full((num_heads,), qk_gain_init, dtype=torch.float32))

        # Per-head λ coefficient (sigmoid ensures λ ∈ (0,1))
        # Initialize following the paper: λ_init ≈ exp(-0.3 * (layer_idx+1))
        import math as _math
        lambda_init = _math.exp(-0.3 * (layer_idx + 1))
        self.lambda_coef = nn.Parameter(
            torch.full((num_heads,), _math.log(lambda_init / (1 - lambda_init + 1e-6)),
                       dtype=torch.float32))  # pre-sigmoid logits

        # RoPE for sub_head_dim (initialized here, may be overridden in GPT.__init__)
        self.rope_dims = 0
        self.rotary = Rotary(self.sub_head_dim, base=rope_base, train_seq_len=train_seq_len)
        self.use_xsa = False
    ...
    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        sdim = self.sub_head_dim

        # Q: [B, T, num_heads, 2*sdim] → split into Q1, Q2
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, 2 * sdim)
        q1, q2 = q[..., :sdim], q[..., sdim:]

        # K: [B, T, num_kv_heads, 2*sdim] → split into K1, K2
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, 2 * sdim)
        k1, k2 = k[..., :sdim], k[..., sdim:]

        # V: [B, T, num_kv_heads, sdim]
        v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, sdim)

        # QK norm (applied per-half)
        q1 = F.rms_norm(q1, (sdim,))
        q2 = F.rms_norm(q2, (sdim,))
        k1 = F.rms_norm(k1, (sdim,))
        k2 = F.rms_norm(k2, (sdim,))

        # RoPE (both halves share the same positional encoding)
        ...
        # Per-head gain (same gain applied to both Q halves of a head)
        gain = self.q_gain.to(dtype=q1.dtype)[None, None, :, None]
        q1 = q1 * gain
        q2 = q2 * gain

        # Two causal attention passes: [B, T, num_heads, sdim]
        y1 = flash_attn_3_func(q1, k1, v, causal=True)
        y2 = flash_attn_3_func(q2, k2, v, causal=True)

        # Optional XSA correction (applied to y1 only — the primary attention path)
        if self.use_xsa:
            y1 = self._xsa_efficient(y1, v)

        # Differential subtraction: y = y1 - λ·y2
        lam = torch.sigmoid(self.lambda_coef).to(dtype=y1.dtype)[None, None, :, None]
        y = y1 - lam * y2  # [B, T, num_heads, sdim]

        y = y.reshape(bsz, seqlen, self.num_heads * sdim)
        return self.proj(y)
    ...
    def __init__(self, h: Hyperparameters):
        ...
        self.blocks = nn.ModuleList([
            Block(h.model_dim, h.num_heads, h.num_kv_heads, h.mlp_mult, h.rope_base,
                  ...
                  parallel=(i >= h.parallel_residual_start), diff_attn=h.diff_attn)
            for i in range(h.num_layers)
        ])
        if h.rope_dims > 0:
            head_dim = h.model_dim // h.num_heads
            # For diff-attn, RoPE is applied to sub_head_dim (= head_dim//2) slices.
            rope_head_dim = (head_dim // 2) if h.diff_attn else head_dim
            # rope_dims applied within the sub_head_dim space (clamp to avoid overflow)
            effective_rope_dims = min(h.rope_dims, rope_head_dim)
            for block in self.blocks:
                block.attn.rope_dims = effective_rope_dims
                block.attn.rotary = Rotary(rope_head_dim, base=h.rope_base,
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        # lambda_coef is a per-head scalar (1-D) → Adam, not Muon
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,lambda_coef",
    ).split(",")
    if pattern
)
