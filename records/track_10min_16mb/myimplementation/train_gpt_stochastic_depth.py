"""Stochastic Depth (LayerDrop) variant of the Megakernel submission.
Builds on the TripleLoop+ParallelResiduals SOTA baseline.

Key idea (from "Deep Networks with Stochastic Depth", Huang et al., 2016 +
          "Reducing Transformer Depth on Demand with Structured Dropout", Fan et al., 2019):
  During training, each transformer block is randomly SKIPPED (its residual delta is
  multiplied by a Bernoulli mask) with a layer-depth-dependent probability.
  Shallower layers (lower risk) are dropped rarely; deeper layers more often.
  A linearly increasing schedule is used:  drop_prob[i] = max_prob * i / (N - 1)

Why it helps:
  1. Acts as strong regularization for small models — prevents over-reliance on any
     single layer's transformation, improving generalization.
  2. Creates an implicit ensemble of networks of varying effective depths, similar to
     dropout creating an ensemble of sub-networks.
  3. The gradient signal reaches early layers more reliably (dropped later layers don't
     block gradients from flowing back through the entire stack).
  4. Particularly valuable for layer-looping architectures: the looped block sees the
     stochastic skip signal, preventing it from over-specializing.

Implementation note — torch.compile compatibility:
  - `self.drop_prob` is a Python float (compile-time constant per block).
  - `self.training` is a guard: compile generates two graphs (train / eval).
  - In the eval graph, all stochastic depth code is compiled away (zero overhead).
  - `torch.bernoulli` on a constant-shape tensor is fully supported under fullgraph=True.

Changes vs. base:
  1. Hyperparameters: stochastic_depth_max_prob (default 0.1)
  2. Block.__init__: drop_prob attribute
  3. Block.forward: DropPath residual scaling
  4. GPT.__init__: linearly assigns drop_prob to each block
"""
import collections
import copy
import glob
...
class Hyperparameters():
    ...
    # Stochastic Depth (LayerDrop) — linearly increasing per-layer drop probability
    stochastic_depth_max_prob = float(os.environ.get('STOCHASTIC_DEPTH_MAX_PROB', 0.1))
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # Stochastic depth drop probability (0 = never drop; compile-time constant per block)
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        # Stochastic depth (DropPath): randomly skip this block's residual during training.
        # Inactive during eval (training=False → path compiled away, zero runtime cost).
        # Rescaled by 1/keep_prob to maintain expected output magnitude (unbiased estimate).
        if self.training and self.drop_prob > 0.0:
            keep_prob = 1.0 - self.drop_prob
            # Per-sample Bernoulli mask: shape [B, 1, 1] for broadcast over [B, T, D]
            mask = torch.bernoulli(
                torch.full((x.size(0), 1, 1), keep_prob, device=x.device, dtype=x.dtype))
            delta = x_out - x_in
            return x_in + delta * (mask / keep_prob)
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        # Assign linearly increasing stochastic depth probabilities.
        # Block 0: drop_prob=0 (always kept); block N-1: drop_prob=max_prob.
        # Deeper layers are dropped more aggressively — they rely on shallower layers
        # and are less critical for overall gradient flow.
        if h.stochastic_depth_max_prob > 0.0:
            for i, block in enumerate(self.blocks):
                block.drop_prob = h.stochastic_depth_max_prob * i / max(h.num_layers - 1, 1)
        if h.rope_dims > 0:
