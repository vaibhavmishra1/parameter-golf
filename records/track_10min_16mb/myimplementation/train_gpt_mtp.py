"""Multi-Token Prediction (MTP) variant of the Megakernel submission.
Builds on the TripleLoop+ParallelResiduals SOTA baseline.

Key idea: during training, the model is also trained to predict tokens at
positions t+2 and t+3 (not just t+1) using auxiliary linear heads that share
the same final hidden state. These heads are TRAINING-ONLY — they are stripped
from the quantized artifact, adding zero bytes to the submission size.

Why it helps:
- Each position's hidden state must encode information about multiple future tokens,
  creating richer representations and stronger gradient signal.
- Effectively ~3× more "useful" gradient per token without 3× more data.
- Shown in "Better & Faster Large Language Models via Multi-token Prediction"
  (Gloeckle et al., 2024) to improve convergence speed.

Changes vs. base:
  1. Hyperparameters: mtp_heads, mtp_lambda
  2. GPT: mtp_linears ModuleList (training-only aux projection heads)
  3. GPT.forward: computes auxiliary MTP losses during training
  4. GPT.forward_logits_and_hidden: returns hidden state for MTP heads
  5. Optimizers: routes MTP linears to Muon (matrix optimizer)
  6. serialize/deserialize: strips MTP linears from the artifact
"""
import collections
import copy
import glob
...
class Hyperparameters():
    ...
    # Multi-Token Prediction (training-only auxiliary heads)
    mtp_heads = int(os.environ.get('MTP_HEADS', 2))        # number of future-token heads (predicts t+2, t+3, ...)
    mtp_lambda = float(os.environ.get('MTP_LAMBDA', 0.3))  # loss weight for auxiliary heads
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        # Training-only auxiliary MTP heads: one per future token offset (t+2, t+3, ...)
        # These project from embedding_dim → vocab_size, independently of the main head.
        # They are STRIPPED from the serialized artifact (zero artifact cost).
        mtp_out_dim = h.embedding_dim  # project from post-head_proj embedding space
        self.mtp_linears = nn.ModuleList([
            CastedLinear(mtp_out_dim, h.vocab_size, bias=False)
            for _ in range(h.mtp_heads)
        ])
        self.mtp_lambda = h.mtp_lambda
    ...
    def _transformer_body(self, input_ids: Tensor) -> Tensor:
        """Run the transformer body, returning hidden state x in embedding space (before final projection)."""
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        x = self._transformer_body(input_ids)
        if self.tie_embeddings:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Main next-token prediction loss (t+1)
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight)
        else:
            logits_proj = self.lm_head(x)
        ...
        # Auxiliary MTP losses (t+2, t+3, ...) — active only during training
        if self.training and len(self.mtp_linears) > 0:
            aux_loss = torch.zeros((), device=x.device, dtype=torch.float32)
            for k, mtp_head in enumerate(self.mtp_linears):
                offset = k + 1  # head k predicts token at t+(k+2)
                # Use hidden states at positions [0 .. T-offset-1] to predict targets[offset .. T-1]
                aux_logits = self.logit_softcap * torch.tanh(
                    mtp_head(x[:, :-offset, :]) / self.logit_softcap)
                aux_targets = target_ids[:, offset:]  # targets shifted by offset
                aux_loss = aux_loss + F.cross_entropy(
                    aux_logits.reshape(-1, aux_logits.size(-1)).float(),
                    aux_targets.reshape(-1), reduction="mean")
            return main_loss + self.mtp_lambda * aux_loss / len(self.mtp_linears)
...
class Optimizers():
    def __init__(self, h: Hyperparameters, base_model: GPT):
        ...
        # MTP linears are training-only matrix weights → route to Muon for best gradient quality.
        # They are NOT quantized/serialized into the artifact.
        for p in base_model.mtp_linears.parameters():
            if p.ndim == 2:
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> tuple[int, int]:
    ...
    # Strip training-only MTP linear heads from the artifact (they add zero bytes).
    sd_cpu = {k: v.detach().cpu() for k, v in base_model.state_dict().items()
              if not k.startswith("mtp_linears")}
...
def deserialize(h: Hyperparameters, device: torch.device) -> GPT:
    ...
    # Only use non-MTP keys as the template for dequantization (MTP heads are absent in artifact).
    ...
    # Load non-MTP weights strictly; MTP heads are left at random init and then cleared.
    eval_model.load_state_dict(deq_state, strict=False)
    eval_model.mtp_linears = nn.ModuleList()  # drop training-only heads from eval model
