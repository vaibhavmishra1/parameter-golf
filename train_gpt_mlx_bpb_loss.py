#!/usr/bin/env python3
"""
Variant of `train_gpt_mlx.py` that optimizes a **BPB-aligned training objective** instead of
plain mean token cross-entropy.

Challenge eval reports `val_bpb` (bits per UTF-8 byte on the validation text). Under a fixed
tokenizer, that is proportional to:

    (sum of per-token NLL in nats) / (total UTF-8 bytes represented by those target tokens)

with the same per-token byte count rule as `eval_val` (base piece length + leading-space byte
when applicable). Gradients match minimizing that ratio over a minibatch because byte counts do
not depend on model parameters:

    d/dθ ( (Σ_i NLL_i) / B ) = (1/B) Σ_i d NLL_i/dθ.

So we use training loss `sum(NLL) * inv_b` with `inv_b = 1 / B` computed from the same LUTs as
validation. **Validation** still uses mean token CE via `GPT.loss` so logged `val_loss` /
`val_bpb` stay identical to the baseline script.

The `train_loss` printed during training is **nats per byte** (≈ `val_bpb * ln(2)` on an ideal
batch); multiply by `1/log(2)` for a rough bits/byte readout comparable to `val_bpb`.

Hard stop: keep this file under 1500 lines (same as `train_gpt_mlx.py`).
"""
...
class GPT(nn.Module):
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Mean token cross-entropy (nats/token). Used for validation and compatibility."""
    ...
    def loss_train_bpb(self, input_ids: mx.array, target_ids: mx.array, inv_b: mx.array) -> mx.array:
        """(Σ NLL) / B in nats/byte: sum of token CE times inv_b = 1/B (bytes from eval LUT)."""
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
            logits = self.softcap(logits_proj)
            ce_sum = nn.losses.cross_entropy(logits.astype(mx.float32), y, reduction="sum")
            return ce_sum * inv_b
...
def _target_byte_count_np(
    input_ids_np: np.ndarray,
    target_ids_np: np.ndarray,
    base_bytes_lut: np.ndarray,
    has_leading_space_lut: np.ndarray,
    is_boundary_token_lut: np.ndarray,
) -> float:
    prev_ids = input_ids_np.reshape(-1)
    tgt_ids = target_ids_np.reshape(-1)
    bytes_np = base_bytes_lut[tgt_ids].astype(np.int64, copy=True)
    bytes_np += (
        has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]
    ).astype(np.int64, copy=False)
    return float(bytes_np.sum())
...
def loss_and_grad_chunked(
    ...
    """BPB-aligned loss: L = (Σ NLL) / B over the full logical microbatch (all chunks)."""
    ...
    for chunk_tokens in chunk_sizes:
        ...
        b_total += _target_byte_count_np(
            x_np, y_np, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut
        )
    inv_b = 1.0 / max(b_total, 1.0)
    inv_b_mx = mx.array(inv_b, dtype=mx.float32)

    loss_value = mx.array(0.0, dtype=mx.float32)
    grad_accum: dict[str, mx.array] | None = None
    for x, y in batches:
        loss, grads = compiled_loss_and_grad(x, y, inv_b_mx)
        loss_value = loss_value + loss.astype(mx.float32)
        grad_accum = accumulate_flat_grads(grad_accum, grads, 1.0)
...
def main() -> None:
    ...
    compiled_loss_and_grad = mx.compile(
        nn.value_and_grad(model, lambda x, y, inv_b: model.loss_train_bpb(x, y, inv_b)),
        inputs=model.state,
        outputs=model.state,
    )
    ...
    log(
        "train_objective:bpb_weighted (sum token CE nats / UTF-8 bytes per eval LUT); "
        "logged train_loss is nats/byte (~ val_bpb * ln 2 on matched data); val_loss still mean token CE"
    )
    ...
    while True:
        ...
        if args.train_log_every > 0 and (step <= 10 or step % args.train_log_every == 0 or stop_after_step is not None):
            log(
                f"step:{step}/{args.iterations} train_nats_per_byte:{train_loss_value:.4f} "
