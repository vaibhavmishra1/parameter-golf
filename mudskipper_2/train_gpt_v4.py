...
class Hyperparameters:
    ...
    # Mudskipper v4: local drop-worst skimming. This is intentionally close to
    # v1: no replay buffer, no model-loss feedback, no async thread.
...
class MudskipperDropScorer:
    ...
    def score_windows(self, windows: list[Tensor]) -> tuple[np.ndarray, int]:
        rows: list[tuple[float, float, float, float, bool, bool]] = []
        nlls = np.empty(len(windows), dtype=np.float64)
        for i, window in enumerate(windows):
            arr = self._arr(window)
            if arr.size == 0:
                nll, rare_frac, unique_ratio, max_frac, repeat_frac = math.log(self.vocab_size), 0.0, 0.0, 1.0, 1.0
            else:
            ...
            rows.append((rare_frac, unique_ratio, max_frac, repeat_frac, recent, junk))
            nlls[i] = nll
        if len(windows) == 0:
            return np.empty(0, dtype=np.float64), 0
        order = np.argsort(nlls)
        ranks = np.empty(len(windows), dtype=np.float64)
        ranks[order] = np.linspace(0.0, 1.0, num=len(windows), endpoint=True) if len(windows) > 1 else 0.5
        scores = np.empty(len(windows), dtype=np.float64)
        junk_count = 0
        for i, (rare_frac, unique_ratio, max_frac, repeat_frac, recent, junk) in enumerate(rows):
            score = (
                0.58 * float(ranks[i])
                + 0.18 * rare_frac
                + 0.12 * unique_ratio
...
class DistributedTokenLoader:
    ...
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device, args: Hyperparameters | None = None):
        ...
        self.scorer = MudskipperDropScorer(args) if self.skip_enabled and args is not None else None
...
def main() -> None:
    ...
    log0(
        ...
        f"recent_hashes:{args.mudskipper_recent_hashes}"
    )
