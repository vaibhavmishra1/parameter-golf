...
class Hyperparameters:
    ...
    mudskipper_pack_only = bool(int(os.environ.get("MUDSKIPPER_PACK_ONLY", "0")))
    mudskipper_scout_start_step = int(os.environ.get("MUDSKIPPER_SCOUT_START_STEP", "800"))
    mudskipper_scout_warm_counts = bool(int(os.environ.get("MUDSKIPPER_SCOUT_WARM_COUNTS", "1")))
...
class MudskipperScout:
    ...
    def observe_windows_fast(self, windows: list[Tensor]) -> None:
        # Warm the unigram confidence model during pack-only steps without ranking.
        ...
        if arrs:
            ...
            self.counts += np.bincount(flat, minlength=self.vocab_size)
            self.total += float(flat.size)
        while len(self.recent_hashes) > self.recent_hash_limit:
            self.recent_hashes.pop(next(iter(self.recent_hashes)))
...
class DistributedTokenLoader:
    ...
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device, args: Hyperparameters | None = None):
        ...
        self.pack_only = bool(args is not None and args.mudskipper_pack_only)
        self.scout_start_step = 0 if args is None else max(int(args.mudskipper_scout_start_step), 0)
        self.scout_warm_counts = bool(args is not None and args.mudskipper_scout_warm_counts)
    ...
    def _next_packed_batch(
        ...
        windows = self.stream.take_windows(global_num_seqs, seq_len)
        self.stats["stream_ms"] += 1000.0 * (time.perf_counter() - t_stream)

        if warm_scout and self.scout is not None:
            t_update = time.perf_counter()
            self.scout.observe_windows_fast(windows)
        ...
        self.stats["candidates"] += global_num_seqs
        self.stats["accepted_base"] += global_num_seqs
        return x, y
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        if self.pack_only:
            return self._next_packed_batch(global_tokens, seq_len, grad_accum_steps, warm_scout=False)
        if self.scout_enabled and self.step < self.scout_start_step:
            return self._next_packed_batch(
                global_tokens,
                seq_len,
                grad_accum_steps,
                warm_scout=self.scout_warm_counts,
            )
    ...
    def stats_line(self, reset: bool = False) -> str:
        ...
        scout_active = self.scout_enabled and not self.pack_only and self.step >= self.scout_start_step
        line = (
            f"mudskipper scout:{int(self.scout_enabled)} active:{int(scout_active)} pack_only:{int(self.pack_only)} "
            f"step:{self.step} start:{self.scout_start_step} batches:{int(s['batches'])} "
            f"packed_b:{int(s['packed_batches'])} scout_b:{int(s['scout_batches'])} warm_b:{int(s['warm_batches'])} "
...
def main() -> None:
    ...
    log0(
        ...
        f"candidate_mult:{args.mudskipper_candidate_mult:.3f} pack_only:{args.mudskipper_pack_only} "
        f"scout_start_step:{args.mudskipper_scout_start_step} warm_counts:{args.mudskipper_scout_warm_counts} "
        f"bucket_count:{args.mudskipper_bucket_count} recent_hashes:{args.mudskipper_recent_hashes} "
        f"nvidia_smi:{args.mudskipper_nvidia_smi}"
    )
